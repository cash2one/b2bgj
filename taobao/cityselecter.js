...
(function() {
	...
	a.prototype = {
		...
		removefn: function(k, h) {
			if (this.fns[k] instanceof Array) {
				var h = fns[k];
				for (var g = 0, f = fns.length; g < f; g++) {
					if (fns[g] === h) {
						break
					}
				}
	...
	d.prototype = {
		...
		addList: function(k, n) {
			...
			k.innerHTML = g + m;
			var l = this;
			MGTOOL.addEvent(MGTOOL.$id("hotPOP"), "mousedown", function(q) {
				MGTOOL.stopEvent(q);
		...
		_focus: function() {
			...
			var f = this;
			MGTOOL.addEvent(this.wrap, "mousedown", function(g) {
				f.firstFocus = true;
				return false
			})
		},
		...
		bindEvent: function() {
			...
			var f = this;
			MGTOOL.addEvent(this.target, "focus", function() {
				f._focus()
			});
			MGTOOL.addEvent(this.target, "blur", function() {
				f._blur()
			})
		}
	};
	...
	b.prototype = {
		...
		_hotTab: function(k, h) {
			...
			for (; g < n; g++) { (function() {
					var i = g;
					MGTOOL.addEvent(m[i], "mousedown", function(p) {
		...
		_vacationHotClick: function(f) {
			var g = f,
			h = this;
			MGTOOL.addEvent(g, "mousedown", function(l) {
				MGTOOL.stopEvent(l);
		...
		_vacationClick: function(g) {
			...
			MGTOOL.addEvent(this.PP, "mousedown", function(l) {
				MGTOOL.stopEvent(l);
				var i = MGTOOL.getTarget(l),
				k = i.nodeName;
				if (i && k === "A") {
		...
		inputChangeSet: function() {
			...
			this._keyClick();
			var g = this;
			MGTOOL.addEvent(this.keyWrap, "mousedown", function(i) {
				g.keyhide = false
			})
		},
		...
		_bindEvent: function() {
			...
			MGTOOL.addEvent(this.target, "keydown", function(g) {
				f._keyDown(g)
			})
		}
	};
