            ...
            widgets:{
                ...
                modules:{
                    ...
                    'trip-autocomplete-v2':{
                        path:'suggest/js/trip-autocomplete-v2.0' + _JS_DEBUG_,
...
YTRIP.addTripModule({
    ...
    //国内特价
		'trip-flight-sale':{
			root : 'apps/et/trip-home/',
			path : 'js/flight-sale-min.js',
			requires:['trip-slide','cookie','base'],
			combine: true
		},
		//酒店搜索
		'hotel-home-search':{
			root : 'apps/et/trip-home/',
			path : 'js/hotel-home-search-v1.3.js',
      requires : ['trip-search-form', 'trip-autocomplete-v2' ,'trip-calendar'],
			combine: true
		},
		//国际特价
		'trip-ie-flight-sale':{
			root : 'apps/et/trip-home/',
			path : 'js/ie-flight-sale.js',
			requires:['trip-slide','base','jsonp'],
			combine: true
		},
		'trip-img-box':{
			root : 'apps/et/trip-home/',
			path : 'js/img-box-min.js',
			requires:['node-base','event-delegate','event-mouseenter','base'],
			combine: true
		},
    //哈勃监控 + 页面点击埋点
    'hubble':{
        fullpath:'http://a.tbcdn.cn/??app/dp/s/monitor/hubble-min.js,s/tb-tracer-min.js',
        requires:['event-base']
    },
		//一级搜索
		'firstclass-search' : {
			root : 'apps/et/travel/agent/',
			path : 'js/firstclass-search-v1.1.js',
			requires : ['node-base','node-event-delegate','jsonp','trip-autocomplete-v2','trip-monitor'],
			combine : true
		}
	}
});/**
 ...
 */
/*KISSY.use('core',function(S){TB.Global.init();});*/
YTRIP.use('trip-autocomplete-v2' ,'trip-search-form','anim','trip-calendar','trip-flight-sale','trip-img-box','trip-lazyload','imageloader' , 'trip-ie-flight-sale',function (Y){
	Y.on('available' , function (){
		//搜索切换
		var searchTab = new Y.Slide('J_search_tab',{
		  autoSlide:false,
		  eventype:'click',
		  effect:'none'
		});
		var sPosEl = Y.one('#J_SearchPosBox');
		...
		searchTab.on('switch',switchHandle);
		//国内机票搜索
		var depJipiaoAc = new Y.TripAutoComplete({
		  inputNode : '#J_jipiao_depCity',
		  codeInputNode : '#J_jipiao_depCity_code',
		  createIcon:true,
      source : 'http://s.jipiao.trip.taobao.com//city_search.do?callback={callback}&lines=10&q=',
      resultListLocator : Y.TripAutoComplete.neConfig.resultListLocator,
      resultFormatter : Y.TripAutoComplete.neConfig.resultFormatter
		});
		var arrJipiaoAc = new Y.TripAutoComplete({
		  inputNode : '#J_jipiao_arrCity',
		  codeInputNode : '#J_jipiao_arrCity_code',
		  createIcon:true,
      source : 'http://s.jipiao.trip.taobao.com//city_search.do?callback={callback}&lines=10&q=',
      resultListLocator : Y.TripAutoComplete.neConfig.resultListLocator,
      resultFormatter : Y.TripAutoComplete.neConfig.resultFormatter
		});
		var jipiaoDate = new Y.TripCalendar({
			 beginNode: '#J_jipiao_depDateBox',
			 endNode: "#J_jipiao_endDateBox",
			 isWeek: true,
			 isFestival: true, 
			 limitBeginDate: new Date()
		});
		var jipiaoFormAction =  new  Y.SearchForm({node:'#J_jipiao_form','defaultCity':'#J_jipiao_depCity','storage' : true});

	} , '#J_slide');

	Y.on('available',function (){
	  var homeSale = new Y.FlightSale({
		node : '#J_jipiao_module',
		count: 10
	  });
	  var homeIeSale = new Y.IeFlightSale({
		node : '#J_Ijipiao_module',
		count : 16
	  });

	  //酒店slide
	  var homeJiudian = new Y.Slide('J_jiudian_module',{selectedClass:'cur'});
	  //酒店图片展示模块
	  var imgBox = new Y.TripImgBox({node:'#J_jiudian_module'});
	  /*
	  焦点图
	  */
	  new Y.Slide('J_slide',{
	  ...
	  //旗舰店滚动
	  var flagShipSlide = new Y.Slide('J_flagship',{
		autoSlide:false,
		eventype:'mouseover',
		effect:'h-slide',
		itemWidth : 74 ,
		navClass:'tab-nav',//导航ul的className,因为这里名称也是tab-nav,因此可以不指定
		pannelClass:'tab-pannel',//每个内容单元div的ClassName
		loop : true
	  });
	  //向后
	  Y.one('#J_flagship_leftbtn').on('click',function(e){
		e.halt();
		flagShipSlide.previous();
	  });
	  //向前
	  Y.one('#J_flagship_rightbtn').on('click',function(e){
		e.halt();
		flagShipSlide.next();
	  });
	  //延迟加载
	  new Y.Lazyload(Y.all('textarea.yui-lazyrender'));
	  var imageloader = new Y.ImgLoadGroup({name: 'fold group',foldDistance: 25 });
	  Y.all('.link-md img , .flagship-md img').each(function(node){
	  ...
	  //TB.Global.writeLoginInfo({"memberServer": "", "outmemServer" : "", "loginServer": "", redirectUrl : "", logoutUrl : ""},true);

	} , '#footer');
	//团购模块随机显示
	Y.on('available',function(){
		var groupList = Y.all('.J_group_list');
		var len = Math.floor(Math.random()*3);
		groupList.item(len).removeClass('hidden');
	},'#J_group_module');
