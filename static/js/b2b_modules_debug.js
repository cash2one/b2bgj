...
'0.0.1', {
	requires: ['autocomplete', 'trip-mustache', 'trip-slide', 'trip-placeholder']
});
	...
	if (storageMode === MODE_HTML5 || storageMode === MODE_GECKO) {
		...
		if (storageMode === MODE_HTML5) {
			...
			Y.mix(StorageLite, {
				...
				getItem: function(key, json) {
					try {
						return json ? JSON.parse(storageDriver.getItem(key)) : storageDriver.getItem(key);
					} catch(ex) {
						return null;
					}
				}
			...
			Y.mix(StorageLite, {
				...
				getItem: function(key, json) {
					try {
						return json ? JSON.parse(storageDriver[key].value) : storageDriver[key].value;
					} catch(ex) {
						return null;
					}
				}
